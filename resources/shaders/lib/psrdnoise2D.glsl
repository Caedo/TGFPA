...
vec3 psrd2d_mod289(vec3 x) {
  return x - floor(x * (1.0 / 289.0)) * 289.0;
}

float psrd2d_mod289(float x) {
  return x - floor(x * (1.0 / 289.0)) * 289.0;
}

// Permutation polynomial (ring size 289 = 17*17)
vec3 psrd2d_permute(vec3 x) {
  return psrd2d_mod289(((x*34.0)+10.0)*x);
}

float psrd2d_permute(float x) {
  return psrd2d_mod289(((x*34.0)+1.0)*x);
}
...
// Map from a line to a diamond such that a shift maps to a rotation.
  float u = psrd2d_permute(psrd2d_permute(p.x) + p.y) * 0.0243902439 + rot; // Rotate by shift
  u = 4.0 * fract(u) - 2.0;
...
vec3 srdnoise(vec2 pos, float rot) {
  ...
  iuw = psrd2d_mod289(iuw);
  ivw = psrd2d_mod289(ivw);
